(function () {
  ...
  $.widget('sidebar', {
    ...
    _addObservers: function () {
      ...
      }).on('click', '.qty-minus', function (event) {
        var qty = parseInt($(this).parent('.qty-wrapper').find('.item-qty').val());
        qty -= 1;
        if (qty <= 0) {
          self._removeItem($(this).parent('.qty-wrapper').find('.item-qty'));
        }
        $(this).parent('.qty-wrapper').find('.item-qty').val(qty).trigger('change');

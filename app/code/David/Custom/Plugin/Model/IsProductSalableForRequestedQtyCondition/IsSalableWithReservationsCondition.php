...
namespace David\Custom\Plugin\Model\IsProductSalableForRequestedQtyCondition;
...
class IsSalableWithReservationsCondition
{
     ...
     */
    public function aroundExecute($subject, $proceed, string $sku, int $stockId, float $requestedQty): ProductSalableResultInterface
    {
        ...
        if (!$isEnoughQty) {
            $errors = [
                $this->productSalabilityErrorFactory->create([
                    ...
                    'message' => __('Sorry we only have ').$qtyLeftInStock.__(' left in stock. Please contact us.')
                ])
            ];
            return $this->productSalableResultFactory->create(['errors' => $errors]);
        }
